...
from apps.ai_core.service import AICoreScoringService, ScoringUnavailableError


@override_settings(ANTHROPIC_API_KEY="", JUDGE_HEURISTIC_FALLBACK_ENABLED=True)
...
@override_settings(ANTHROPIC_API_KEY="", JUDGE_HEURISTIC_FALLBACK_ENABLED=False)
def test_score_text_raises_when_heuristic_fallback_disabled():
    with pytest.raises(ScoringUnavailableError):
        AICoreScoringService.score_text("Sample contribution text for scoring.")